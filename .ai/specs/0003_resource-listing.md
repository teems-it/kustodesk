...
- **Errata (2026-09-03):** Some clusters reject `.show materialized views` outright (400 with a parser-level `SYN0002` recognition error — observed cluster-wide on a production ADX cluster that *does* contain materialized views, visible via `.show database schema as json`). `getResources` therefore fetches both lists independently via `Promise.allSettled`, and when the MV command fails it retries via `.show database schema as json` (extracting `Databases[db].MaterializedViews` keys) before degrading to an empty list. A `.show tables` failure remains fatal. Kusto error bodies are surfaced in error envelopes via `describeKustoError` instead of the generic axios message.
